...
---@class tdBag2FrameBase: Frame
---@field private meta tdBag2FrameMeta
local FrameBase = ns.Addon:NewClass('UI.FrameBase', 'Frame')

function FrameBase:Constructor(_, bagId)
...
function FrameBase:OnShow()
    PlaySound(SOUNDKIT_IG_BACKPACK_OPEN)
end

function FrameBase:OnHide()
    PlaySound(SOUNDKIT_IG_BACKPACK_CLOSE)
    self.meta.owner = nil
    self:UnregisterAllEvents()
end

FrameBase.OnSizeChanged = ns.Spawned(UpdateUIPanelPositions)

function FrameBase:UpdatePosition()
...
function FrameBase:SavePosition()
    if not self.meta.profile.managed then
        LibWindow.SavePosition(self)
    end
end

function FrameBase:UpdateManaged()
...
function FrameBase:UpdateSpecial()
    if not self.meta.profile.managed then
